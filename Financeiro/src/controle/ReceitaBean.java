...
public class ReceitaBean extends AbstractBean implements Serializable {
	...
	public void somaReceita() {
		...
		Double somaFarda = rfDAO.soma(this.mes, this.ano);
		if (somaFarda == null) {
			somaFarda = new Double(0);
		}
		ru.setTotal(somaFarda);
		ru.setNome("Farda");
		this.receitaUnica.add(ru);
		ru = new ReceitaUnica();

		ReceitaTonerDAO rtDAO = new ReceitaTonerJPADAO();
		Double somaToner = rtDAO.soma(this.mes, this.ano);
		if (somaToner == null) {
			somaToner = new Double(0);
		}
		ru.setTotal(somaToner);
		ru.setNome("Toner");
		this.receitaUnica.add(ru);
		ru = new ReceitaUnica();

		this.receitaTotal = somaMatri.doubleValue() + somaExtra.doubleValue() + somaLivro.doubleValue()
				+ somaMensa.doubleValue() + somaFarda.doubleValue() + somaToner.doubleValue();
	}
